...
curl -s -o services.json https://raw.githubusercontent.com/antoinelochet/cache-domains/master/cache_domains.json
...
#    fi
    ...
    if ! env | grep "DISABLE_${SERVICEUC}=true" >/dev/null 2>&1; then
      curl -s -o ${L} https://raw.githubusercontent.com/antoinelochet/cache-domains/master/${L}
